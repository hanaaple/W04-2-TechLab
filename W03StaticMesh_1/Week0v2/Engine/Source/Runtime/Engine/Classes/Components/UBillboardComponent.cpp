...
FMatrix UBillboardComponent::CreateBillboardMatrix()
{
	...
	FVector worldLocation = GetLocalLocation();
	if (m_parent) worldLocation = GetLocalLocation() + m_parent->GetWorldLocation();
	FVector worldScale = GetLocalScale();
...
bool UBillboardComponent::CheckPickingOnNDC(const TArray<FVector>& checkQuad, float& hitDistance)
{
	...
	for (int i = 0; i < checkQuad.Num(); i++)
	{
        ...
        FVector v = FVector(checkQuad[i].x, checkQuad[i].y, checkQuad[i].z);
